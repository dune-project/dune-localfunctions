...
int main ( int argc, char **argv )
{
  ...
  // typedef Dune::AlgLib::MultiPrecision<128> Field;
  typedef double Field;
  const int dimension = Topology::dimension;

  typedef Dune::MultiIndex< dimension > MIF;
  typedef Dune::MonomialBasis< Topology, MIF > Basis;
  Basis basis;
  typedef Dune::MultiIndexEvaluator<Basis,Field> Evaluator;
  Evaluator eval(basis,p);
  typedef Evaluator::Iterator<1>::All Iterator;
  ...
  {
    Iterator iter = eval.evaluateAll<1>(it->point());
