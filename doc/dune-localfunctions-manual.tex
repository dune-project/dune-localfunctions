...
\item The {\em normal orientation} of faces of a grid element.  This is useful
  ...
  generally not be extracted from the topology and geometry information alone.
\end{enumerate}
...
use the topological information derived from the global ids of the vertices of
the element instead of vertex indices inside the reference element:
\begin{lstlisting}
unsigned global_orientation[2];
global_orientation[0] = topo[local_orientation[0]];
global_orientation[1] = topo[local_orientation[1]];
\end{lstlisting}
...
create a tailored finite element: geometry, topology and face orientation.
...
encoding given earlier.  If neither geometry nor topology is required, but the
geometry type is, that should be given in place of geometry and topology
directly.  Any extra information should be given after these arguments.  The
possible signatures for make thus are:
\begin{lstlisting}
make(const Geometry&, const Topology&, const FaceOrient&, ...);
make(const Geometry&, const Topology&, ...);
make(const Geometry&, const FaceOrient&, ...);
make(const Geometry&, ...);
make(const Topology&, const FaceOrient&, ...);
make(const Topology&, ...);
make(const GeometryType&, const FaceOrient&, ...);
make(const GeometryType&, ...);
make(const FaceOrient&, ...);
make(...);
\end{lstlisting}
