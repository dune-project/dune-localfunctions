...
{
  typedef typename DualLfe::Traits::LocalBasisType::Traits DualTraits;
  typedef typename LagrangeLfe::Traits::LocalBasisType::Traits LagrangeTraits;

  const int dim = DualTraits::dimDomain;
  assert (dim == LagrangeTraits::dimDomain);

  typedef typename DualTraits::DomainFieldType ctype;
  typedef typename DualTraits::RangeFieldType field_type;

  typedef Dune::ReferenceElement<ctype,dim> RefElement;
  const RefElement& refElement = Dune::ReferenceElements<ctype,dim>::general(dualLfe.type());
  ...
  for (int i=0; i<refElement.size(1); i++) {
    ...
    const Dune::QuadratureRule<ctype,dim-1>& quad = Dune::QuadratureRules<ctype,dim-1>::rule(refElement.type(i,1),  dualLfe.localBasis().order() + lagrangeLfe.localBasis().order());
    ...
    for(size_t j=0; j < quad.size(); j++) {
      ...
      const Dune::FieldVector<ctype,dim-1>& pos = quad[j].position();
      const Dune::FieldVector<ctype,dim>& elementPos = geometry.global(pos);
