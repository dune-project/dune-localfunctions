...
bool testJacobian(const FE& fe,
                  unsigned order = 2,
                  const std::function<bool(const typename FE::Traits::LocalBasisType::Traits::DomainType&)> pointSkip = [](const typename FE::Traits::LocalBasisType::Traits::DomainType&){return false;}
)
{
...
struct TestPartial
{
                  ...
                  const std::function<bool(const typename FE::Traits::LocalBasisType::Traits::DomainType&)> pointSkip = [](const typename FE::Traits::LocalBasisType::Traits::DomainType&){return false;})
  {
    ...
    if (diffOrder >= 2)
      success = success and testOrder2(fe, eps, delta, order, pointSkip);

    if (diffOrder >= 1)
      success = success and testOrder1(fe, eps, delta, order, pointSkip);
 ...
 *
 * \param pointSkip This is a small predicate class that allows to skip certain
           ...
           )
{
  ...
  {
    success = TestPartial::test(fe, TOL, jacobianTOL, diffOrder, quadOrder, pointSkip) and success;
  }
  ...
  if (not (disabledTests & DisableVirtualInterface))
  {
    ...
    if (not (disabledTests & DisableJacobian))
    {
      success = testJacobian<VirtualFEInterface>(virtualFE, quadOrder, pointSkip) and success;
    }
    else
    {
      // make sure diffOrder is 0
