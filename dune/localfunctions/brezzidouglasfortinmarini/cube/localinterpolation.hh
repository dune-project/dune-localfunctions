...
namespace Dune
{
  ...
  class BDFMCubeLocalInterpolation
  {
    ...
    static constexpr auto faceDofs = std::intergal_constant<unsigned int, binomial(dim+order-2, order-1)>{};
  ...
  template<class D, class R, unsigned int dim, unsigned int order>
  constexpr std::size_t BDFMCubeLocalInterpolation<D, R, dim, order>::2*dim;
